...
df <- datos %>% select(ds = date, y = ads_spend)
holidays <- read_csv("holidays.csv")

# make forecasts
m <- prophet(df, changepoint.range = .5, seasonality.mode = 'multiplicative', holidays = holidays)
...
# save
write.csv(forecast[c('ds', 'yhat', 'yhat_lower', 'yhat_upper')], "11_final_forecasts/forecast_ads_spend.csv", row.names = F)

p <- plot(m, forecast) + add_changepoints_to_plot(m) 
png(paste0("11_final_forecasts/changepoints_ads_spend.png"), width = 800, height = 500)
print(p)
dev.off()

png(paste0("11_final_forecasts/components_ads_spend.png"), width = 800, height = 500)
prophet_plot_components(m, forecast) 
dev.off()