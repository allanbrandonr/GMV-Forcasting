...
d_grid <- read.csv("10_optimize_forecast/all_forecasts_gmv.csv") %>% arrange(mape_20h1_30)
View(d_grid)
i <- 1
...
# save
write.csv(pronos$forecast[c('ds', 'yhat', 'yhat_lower', 'yhat_upper')], "11_final_forecasts/forecast_gmv.csv", row.names = F)

p <- plot(pronos$m, pronos$forecast) + add_changepoints_to_plot(pronos$m) + coord_cartesian(ylim = c(-25000,250000))
png(paste0("11_final_forecasts/changepoints_gmv.png"), width = 800, height = 500)
print(p)
dev.off()

png(paste0("11_final_forecasts/components_gmv.png"), width = 800, height = 500)
prophet_plot_components(pronos$m, pronos$forecast) 
dev.off()
